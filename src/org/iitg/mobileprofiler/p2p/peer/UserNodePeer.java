...
public class UserNodePeer extends Peer {
	...
	/**
	 * This is the number of peers to which you wish to connect to (max) 0 -
	 * infinite.
	 */
	...
	public UserNodePeer(String key, String peerName, int peerPort,
			ArrayList<Integer> userClassContents, String bootstrapInfo,
			String SBCAddress, int numOfPeers) {
	...
	protected void onReceivedJSONMsg(JSONObject peerMsg, Address sender) {
		try {
			JSONObject params = peerMsg.getJSONObject("payload").getJSONObject(
					"params");
			...
			if (peerMsg.get("type").equals(UserQueryMessage.MSG_USER_QUERY)) {
				...
				ArrayList<Integer> questionClassDistribution = UtilityFunctions
						.getClassDistributionFromString(peerMsg.get(
								"classDistribution").toString());
				Double similarity = UtilityFunctions.getSimilarityScore(
						questionClassDistribution, classContents);
			...
			if (peerMsg.get("type").equals(QueryReplyMessage.MSG_QUERY_REPLY)) {
				...
				Double similarity = Double.parseDouble(peerMsg
	...
	public void sendPeerListRequestMessage() {
		if (bootstrapAddress != null) {
			PeerListRequestMessage peerListRequestMessage = new PeerListRequestMessage(
					peerDescriptor);
